...
new_func = """function renderSubjectGroupView() {
  ...
  sortedSubjects.forEach(sub => {
    ...
    item.innerHTML = `
      <div style="display: flex; align-items: center; justify-content: space-between; width: 100%;">
        ...
        <div style="display: flex; align-items: center; gap: 6px;">
          <span style="font-size: 11px; color: var(--text-secondary); background: var(--bg-secondary); padding: 2px 6px; border-radius: 10px;">${records.length}명</span>
          <i data-lucide="${iconType}" style="width: 14px; height: 14px; color: ${iconColor};"></i>
        </div>
      </div>
    `;
    ...
    item.addEventListener('click', () => {
      ...
      if (metaSpan) metaSpan.textContent = `학생 ${records.length}명 / 총 오류 ${totalErrors}건`;
      ...
      if (cardsContainer) {
        ...
        sortedRecords.forEach(r => {
          ...
          const statusTxt = eCount > 0 ? `오류 ${eCount}` : '통과';
          ...
          const studentMeta = sNumStr ? `${grade} ${classVal} - ${sNumStr}` : '학번 없음';
          
          card.innerHTML = `
            <div class="group-card-title">
              <span>${escapeHTML(r.name || '이름 없음')}</span>
          ...
          card.addEventListener('click', () => {
            ...
            if (actualIndex !== -1) {
              ...
              const tab = document.querySelector('.menu-item[data-tab="student-group-view"]');
              if (tab) tab.click();
              showToast(`'${r.name}' 학생 상세 화면으로 이동했습니다.`);
            }
          });
